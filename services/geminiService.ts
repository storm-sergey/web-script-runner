...
import { LogEntry, ScriptDef } from "../types";

const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });

/**
 * Constructs the command line string based on script definition and form values.
 */
const constructCommand = (scriptDef: ScriptDef, args: Record<string, any>, isDryRun: boolean): string => {
  ...
  // Add mapped arguments
  scriptDef.fields.forEach(field => {
    if (field.argFlag && args[field.key]) {
      // Handle quoting for strings containing spaces
...
/**
 * Attempts to execute the script on a local backend, falling back to AI simulation
 * if the backend is not reachable (demo mode).
 */
export const executeScript = async (
  ...
  try {
    // 1. Attempt to call the actual local execution engine
    // This assumes a backend is running at localhost:8000/api/execute
    // Since this is a demo environment, this will likely fail, triggering the catch block.
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 1000); // Fast fail for demo
    ...
    // 2. Fallback: Simulate the output using Gemini based on the EXACT command constructed.
    console.warn("Backend unavailable, using AI simulation.", backendError);
...
const simulateOutput = async (command: string, scriptDef: ScriptDef, isDryRun: boolean): Promise<LogEntry[]> => {
  ...
  const prompt = `
    ...
    4. IF script name includes "triggers", explicitly mention Zabbix Trigger IDs in the logs (e.g. "Disabled Zabbix Trigger ID 10442").
  ...
  try {
    ...
    return [
       { timestamp: '', level: 'ERROR', message: 'Simulation Error: Failed to generate output.' }
    ];
  }
}